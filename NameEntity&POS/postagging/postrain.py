...
DEV_FORMATTED_FILENAME = 'pos_dev_formatted.temp'
INPUT_FORMATTED_FILENAME = 'pos_input_formatted.temp'
...
def InputFormat(input_path, dev_or_input):
    ...
    with open(temp_file, 'w') as pos_formatted_file:
        with open(input_path, 'r') as input_file:
            for line in input_file:
                words_and_tags = line.split()
                words_and_tags.insert(0, 'BOS/BOS')
                words_and_tags.append('EOS/EOS')
                for i in range(1, len(words_and_tags) - 1):
                    ...
                    if len(word) >= 3:  
                        ...
                        surfix3 = word
                    tag = word_tag_pair[1]
                    pre_word = words_and_tags[i-1].split('/')[0]
                    next_word = words_and_tags[i+1].split('/')[0]
                    formatted_line = tag + ' ' + 'pre:' + pre_word + ' ' + 'w:' + word + ' ' + 'next:' + next_word + ' ' + 'wshape:' + wshape + ' ' + 'surfix2:' + surfix2 + ' ' + 'surfix3:' + surfix3 + '\n'
                    pos_formatted_file.write(formatted_line)
